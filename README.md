...
```bash
flet pack main.py -D -y -n main --product-name "File Translator"
```

`-D` produces a one-folder bundle at `dist/main/main.exe`, which is the layout the installer expects.

Then, to publish:

1. Zip the contents of `dist/main` as `build.zip`.
2. Compile `Translator_Installer.iss` with Inno Setup to produce `TranslatorInstaller.exe`.
3. Create a GitHub Release and attach **both** `build.zip` and `TranslatorInstaller.exe`. The installer downloads `build.zip` from `releases/latest` at install time, which keeps the installer itself small.
4. **Only then** bump the version in all three places, together:
   - `CURRENT_VERSION` in [`main.py`](main.py)
   - `version` in [`version.json`](version.json)
   - `AppVersion` in [`Translator_Installer.iss`](Translator_Installer.iss)

> ⚠️ Bump `version.json` **last**. Installed clients poll it on startup and prompt to update as soon as it exceeds their own version. If the release artifacts aren't published yet, users download the old installer, end up back on the same version, and get prompted again on every launch.
...
└── Translator_Installer.iss   # Inno Setup script
```
